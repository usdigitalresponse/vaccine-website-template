...
api.ColorsTool = api.Control.extend({
	...
	initPatterns: function() {
		...
		ct.patternPicker.on( 'click', '.pattern a', function ( e ) {
			e.preventDefault();
	...
	initPalettes: function() {
		...
		$( '#more-palettes' ).on( 'click', function () {
...
api.ColorsTool.BackgroundChangeView = wp.Backbone.View.extend({
	...
	initialize: function() {
		_.bindAll(this, 'updateImage', 'updateBgSize', 'updateRectangleStyle',
			'showPickerBorder', 'hidePickerBorder');
...
api.ColorsTool.BackgroundOptionsView = wp.Backbone.View.extend({
	...
	states: function() {
		return _.reduce(this._defaults, function(acc, val, key) {
	...
	render: function() {
		this.$el.html(this.template());
		_.each(this.states(), function(value, key) {
